...
typedef enum octaspire_lightboard_state_tween_direction_t
{
    OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_OUT,
    OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_IN,
    OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_NONE
}
octaspire_lightboard_state_tween_direction_t;

struct octaspire_lightboard_state_t
{
    octaspire_memory_allocator_t           *allocator;
    octaspire_lightboard_game_t                  *parent;
    octaspire_dern_vm_t                    *vm;
    bool                                    done;
    octaspire_container_hash_map_t         *animations;
    octaspire_lightboard_state_tween_direction_t  tweenTo;
    int                                     origoX;
    int                                     winW;
    int                                     shakeX;
    int                                     shakeCounter;
...
octaspire_dern_value_t *octaspire_lightboard_api_shake(
    octaspire_dern_vm_t *vm,
...
octaspire_dern_value_t *octaspire_lightboard_api_animation_entity_get_x(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    octaspire_container_hash_map_element_t * element = octaspire_container_hash_map_get(
        state->animations,
        octaspire_dern_value_get_hash(firstArg),
    ...
    octaspire_sdl2_animation_t *animation = octaspire_container_hash_map_element_get_value(element);

    return octaspire_dern_vm_create_new_value_integer(vm, octaspire_sdl2_animation_get_x(animation));
}

octaspire_dern_value_t *octaspire_lightboard_api_animation_entity_get_y(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    return octaspire_dern_vm_create_new_value_integer(vm, octaspire_sdl2_animation_get_y(animation));
}

octaspire_dern_value_t *octaspire_lightboard_api_animation_entity_new(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    if (!octaspire_lightboard_state_add_named_animation_of_type_to_position(
            state,
...
octaspire_dern_value_t *octaspire_lightboard_api_animation_entity_remove(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    if (!octaspire_lightboard_state_remove_named_animation(
            state,
            octaspire_container_utf8_string_get_c_string(animNameVal->value.string)))
    {
        abort();
...
octaspire_dern_value_t *octaspire_lightboard_api_tween_level_out(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    if (!octaspire_lightboard_state_tween_level_out(state))
    {
        abort();
    }
...
octaspire_dern_value_t *octaspire_lightboard_api_animation_entity_remove_all(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    if (!octaspire_lightboard_state_remove_all_animations(state))
    {
        abort();
    }
...
octaspire_dern_value_t *octaspire_lightboard_api_animation_entity_set_position(
    octaspire_dern_vm_t *vm,
    octaspire_dern_value_t *arguments,
    octaspire_dern_value_t *environment)
{
    ...
    if (!octaspire_lightboard_state_set_animation_position(
            state,
            octaspire_container_utf8_string_get_c_string(animNameVal->value.string),
            animPosX,
            animPosY,
...
{
    octaspire_lightboard_state_t *self =
        octaspire_memory_allocator_malloc(allocator, sizeof(octaspire_lightboard_state_t));
    ...
    if (!self->vm)
    {
        octaspire_lightboard_state_release(self);
        self = 0;
        return 0;
    }
    ...
    self->tweenTo = OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_NONE;
    self->origoX  = 0;
    self->winW    = 0;
    self->shakeX  = 0;
    self->shakeCounter = 0;
    ...
    int const winH)
{
    ...
    if (self->tweenTo == OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_OUT)
    {
        if (self->tweenUptime >= 1.25)
        {
            self->tweenTo = OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_IN;
            self->tweenUptime = 0;
            self->origoX = 0;
    ...
    else if (self->tweenTo == OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_IN)
    {
        if (self->tweenUptime >= 1.25)
        {
            self->tweenTo = OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_NONE;
    ...
    tmpVal = octaspire_dern_vm_create_new_value_integer(self->vm, mouseX);
    octaspire_dern_value_as_vector_push_back_element(updateCallVal, &tmpVal);
    tmpVal = octaspire_dern_vm_create_new_value_integer(self->vm, mouseY);
    octaspire_dern_value_as_vector_push_back_element(updateCallVal, &tmpVal);
    // Origo
    tmpVal = octaspire_dern_vm_create_new_value_integer(self->vm, origoX);
    octaspire_dern_value_as_vector_push_back_element(updateCallVal, &tmpVal);
    tmpVal = octaspire_dern_vm_create_new_value_integer(self->vm, origoY);
    octaspire_dern_value_as_vector_push_back_element(updateCallVal, &tmpVal);
...
bool octaspire_lightboard_state_tween_level_out(
    octaspire_lightboard_state_t * const self)
{
    if (self->tweenTo == OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_NONE)
    {
        self->tweenTo = OCTASPIRE_LIGHTBOARD_STATE_TWEEN_DIRECTION_OUT;
        return true;
    }
...
void octaspire_lightboard_state_private_render_layer(
    ...
    int const origoY)
{
    ...
    for (int i = -1; i < 2; ++i)
    {
        octaspire_lightboard_state_private_render_layer(self, i, renderer, texture, newOrigoX, origoY);
    }
}
